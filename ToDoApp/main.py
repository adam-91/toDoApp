...
async def read_all_activities(db:  Annotated[Session, Depends(get_db)]):
    return db.query(Activities).all()

@app.get('/activity/{activity_id}', status_code = status.HTTP_200_OK)
async def read_activity(db:  Annotated[Session, Depends(get_db)], activity_id: int = Path(gt=0)):
...
async def create_activity(db: Annotated[Session, Depends(get_db)], activities_request: ActivitiesRequest):
    activities_model = Activities(**activities_request.model_dump())

    db.add(activities_model)
    db.commit()

@app.put('/activity/{activity_id}', status_code=status.HTTP_204_NO_CONTENT)
async def update_activity(db: Annotated[Session, Depends(get_db)], 
...
@app.delete('/activity/{actvity_id}', status_code=status.HTTP_204_NO_CONTENT)
async def delete_actvity(db: Annotated[Session, Depends(get_db)], activity_id: int = Path(gt=0)):
    actvity_model = db.query(Activities).filter(Activities.id == activity_id).first()

    if actvity_model is None:
        raise HTTPException(status_code=404, detail='Actitivy not found')
